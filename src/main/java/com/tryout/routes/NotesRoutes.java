...
public class NotesRoutes implements RouteRegistrar {
	public void routes() {
		get(new JsonTransformerRoute("/note/:id") {
			@Override
		...
		get(new JsonTransformerRoute("/note") {
			@Override
			public Object handle(final Request request, final Response response) {
				return DB.dbi().withHandle(new HandleCallback<List<Note>>() {
					public List<Note> withHandle(Handle handle)
							throws Exception {
						...
						if (rs != null && rs.size() > 1) {
							for (Map<String, Object> record : rs) {
								Note note = new Note();
								note.setId(((Integer) record.get("id"))
										.longValue());
		...
		post(new JsonTransformerRoute("/note") {
			@Override
			public Object handle(final Request request, final Response response) {
				Gson gson = new Gson();
						...
						new HandleCallback<Note>() {
							public Note withHandle(Handle handle)
								...
								handle.createStatement(
										...
										.bind("id", DB.nextId())
										.bind("note", note.getNote()).execute();
								note.setId(id);
								return note;
							}
						});
